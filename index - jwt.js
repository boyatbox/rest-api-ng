...
var dbConfig = {
	driver: 'msnodesqlv8',
	connectionString: 'Driver={SQL Server Native Client 11.0};Server=.\\SQLEXPRESS;Database=Testdb;Trusted_Connection=yes;',
};
...
app.post('/api/login', (req, res) => {
	...
	var request = new sql.Request();
	query="select User_Email,User_Role_Name,User_Password_Encrypt from testdb.dbo.[Users]";
	request.query(query, function (err, responseResult) {
		if (err) {
			console.log("Error while executing query :- " + err);
			//res.send(err);
			...
			console.log("Execute query: success");
			dbUsers=responseResult.recordsets[0];
			
			console.log(req.body);
			//console.log(dbUsers);
			console.log(`username=${username} password=${password}`);
			const user = dbUsers.find(u => { return u.User_Email === username && u.User_Password_Encrypt === password });
			if (user) {
				// Generate an access token
				const accessToken = jwt.sign({ username: user.User_Email,  role: user.User_Role_Name}, accessTokenSecret);
...
app.post('/api/test', authenticateJWT, (req, res) => {
